...
import { logsService } from "../services/firestoreService";
...
export default function LogsPage() {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        setLoading(true);
        const data = await logsService.getRecentLogs(100);
        setLogs(data);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="page-sub">
        Audit trail of all admin, seller, and user activities
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-navy-700">
              {["Timestamp", "Actor", "Event Type", "Action", "IP Address"].map(
                (h) => (
                  <th
                    key={h}
