...
export default function AdminsPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", role: "Admin", permissions: "" });

  const addAdmin = () => {
    if (!form.name || !form.email) { toast("Please fill in name and email"); return; }
  ...
  return (
    <div>
      ...
      <div className="page-sub">Manage admin accounts and role-based permissions</div>
      ...
      {showForm && (
        <div className="section-card mb-5 max-w-lg">
          <div className="text-sm font-semibold text-slate-200 mb-4">New Admin Account</div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="email" className="form-input" placeholder="jane@bookvault.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
            </div>
            <div>
            ...
            <div>
              ...
              <input className="form-input" placeholder="e.g. Books, Orders" value={form.permissions} onChange={(e) => setForm({ ...form, permissions: e.target.value })} />
            </div>
          </div>
          <button className="btn-primary mt-4" onClick={addAdmin}>Add Admin</button>
        </div>
      )}
          ...
          <tbody>
            {admins.map((a, i) => (
              <tr key={i} className="border-t border-navy-500 hover:bg-navy-700/40 transition-colors">
                ...
                <td className="px-4 py-3"><Badge status={a.role} /></td>
                <td className="px-4 py-3 text-sm text-slate-300">{a.permissions}</td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {a.role === "Super Admin" ? (
                    <span className="text-xs text-slate-500 italic">Protected account</span>
                  ) : (
                    <>
                      <button className="btn-action" onClick={() => toast(`Edit form for ${a.name} opened`)}>Edit</button>
                      <button
      ...
      <div className="section-card mt-5">
        <div className="text-sm font-semibold text-slate-200 mb-3">Role Permissions Matrix</div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-slate-400">
                <th className="text-left py-2 pr-4 font-medium">Feature</th>
                <th className="text-center py-2 px-4 font-medium">Super Admin</th>
                ...
                <tr key={feature} className="border-t border-navy-500">
                  ...
                  {[sa, a, m].map((v, i) => (
                    <td key={i} className="py-2 px-4 text-center">
                      <span className={v ? "text-green-400" : "text-slate-600"}>{v ? "✓" : "✕"}</span>
                    </td>
                  ))}
                </tr>
              ))}
